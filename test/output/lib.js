...
define(function () {
    ...
    var typeOf = lib.typeOf = function (obj) {
        ...
        return obj && typeof obj === 'object' && 'nodeType' in obj ? 'dom' : obj === null ? null : type;
    };
    /* ========================== lib.array ========================== */
    /**
     * 遍历数组方法
     ...
     * @return {Array} map 处理后的原数组
     */
    var map = lib.map = lib.array.map = fallback(Array.prototype.map, function (obj, iterator, bind) {
     ...
     * @return {number} 指定元素的索引位置，查询不到时返回-1
     */
    var indexOf = lib.indexOf = lib.array.indexOf = fallback(Array.prototype.indexOf, function (source, item, from) {
    ...
    var enumerables = {
        valueOf: 1
    }.propertyIsEnumerable('valueOf') ? null : [
        'hasOwnProperty',
        'valueOf',
        'isPrototypeOf',
     ...
     * @return {JSON} 解析结果 JSON 对象
     */
    lib.parse = lib.object.parse = window.JSON && JSON.parse || function (string) {
        return !string || typeOf(string) !== 'string' ? null : eval('(' + string + ')');
    };
    /**
     * 将对象解析成 query 字符串
     *
    ...
    lib.pad = lib.string.pad = function (source, width) {
        ...
        return (source < 0 ? '-' : '') + (str.length >= width ? str : ((1 << width - str.length).toString(2) + str).slice(-width));
    };
    /**
     * 使用对象数据替换字符串相应的占位字符
     *
    ...
    lib.substitute = lib.string.substitute = function (tpl, data, regexp) {
        return String(tpl).replace(regexp || /\\?\{([^{}]+)\}/g, function (match, name) {
            var value = data[name];
            return match.charAt(0) === '\\' ? match.slice(1) : value !== null && typeof value !== 'undefined' ? value : '';
        });
    };
    /* ========================== lib.fn ========================== */
    /**
     ...
     * @return {Function} 封装后的函数
     */
    lib.bind = lib.fn.bind = fallback(Function.bind, function (fn, scope) {
        var args = arguments.length > 2 ? slice(arguments, 2) : null, F = function () {
        };
        var bound = function () {
            ...
            var result = !args && !length ? fn.call(context) : fn.apply(context, args && length ? args.concat(slice(arguments)) : args || arguments);
        ...
        function implement(newClass, params) {
            ...
            forIn(params, function (value, key) {
                if (lib.isObject(prototype[key]) && lib.isObject(value)) {
                    prototype[key] = lib.extend(lib.clone(prototype[key]), value);
                } else {
                    prototype[key] = value;
                }
            });
     ...
     * @return {(HTMLElement | window)} 目标元素
     */
    lib.on = lib.event.on = document.addEventListener ? function (element, type, listener) {
     ...
     * @return {(HTMLElement | window)} 目标元素
     */
    lib.un = lib.event.un = document.removeEventListener ? function (element, type, listener) {
        var condition = listener;
        var custom = eventFix.custom[type];
        var realType = type;
        if (custom) {
        ...
        element.removeEventListener(realType, condition, !!arguments[3]);
        return element;
    } : function (element, type, listener) {
     ...
     * @return {(HTMLElement | window)} 目标元素
     */
    lib.fire = lib.event.fire = document.createEvent ? function (element, type) {
        var custom = eventFix.custom[type];
        var realType = type;
        if (custom) {
            realType = custom.base;
     ...
     * @param event 事件对象
     */
    lib.preventDefault = lib.event.preventDefault = fallback(window.Event && Event.prototype.preventDefault, function () {
     ...
     * @param event 事件对象
     */
    lib.stopPropagation = lib.event.stopPropagation = fallback(window.Event && Event.prototype.stopPropagation, function () {
    ...
    if (!('onmouseenter' in document)) {
        var check = function (event) {
            ...
            return related !== this && related.prefix !== 'xul' && this.nodeType !== 9 && !lib.dom.contains(this, related);
        };
     ...
     * @return {Array.<HTMLElement>} 符合条件的元素数组
     */
    lib.q = lib.dom.q = document.getElementsByClassName ? function (className, scope) {
     ...
     * @return {?HTMLElement} 符合条件的最近的祖先元素，查找不到时返回 null
     */
    lib.getAncestorBy = lib.dom.getAncestorBy = function (element, condition, arg) {
     ...
     * 查找不到时返回null
     */
    lib.getAncestorByClass = lib.dom.getAncestorByClass = function (element, className) {
     ...
     * @return {boolean} 是否拥有指定的className
     */
    lib.hasClass = lib.dom.hasClass = hasClassList ? function (element, className) {
     ...
     * @return {HTMLElement} 目标元素 `element`
     */
    lib.addClass = lib.dom.addClass = hasClassList ? function (element, className) {
     ...
     * @return {HTMLElement} 目标元素
     */
    lib.removeClass = lib.dom.removeClass = hasClassList ? function (element, className) {
     ...
     * @return {HTMLElement} 目标元素
     */
    lib.toggleClass = lib.dom.toggleClass = hasClassList ? function (element, className) {
        element.classList.toggle(className);
        return element;
    } : function (element, className) {
        var originClass = element.className;
        var contains = originClass && lib.contains(element, className);
        element.className = contains ? originClass.replace(new RegExp('(^|\\s)' + className + '(?:\\s|$)'), '$1') : originClass + ' ' + className;
        return element;
    };
    /**
     * 显示目标元素
    ...
    lib.extend(lib.dom, /** @lends lib.dom */
 {
        /**
         * 获取目标元素的上一个兄弟元素节点
         *
        ...
        contains: function (container, contained) {
            ...
            return container.contains ? container !== contained && container.contains(contained) : !!(contained.compareDocumentPosition(container) & 8);
        }
    });
    return lib;
});
