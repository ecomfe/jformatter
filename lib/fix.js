...
exports.fix = function (source, fixConfig) {
    ...
    rocambole.recursive(ast, function (node) {
        ...
        if (fixConfig.addCurly) {
            require('./rules/addCurly').fix(node);
        }

        if (fixConfig.removeDebugger) {
            require('./rules/removeDebugger.js').fix(node);
        }

        if (fixConfig.noElseReturn) {
            require('./rules/noElseReturn.js').fix(node);
        }
    });
